...
import { ToolActivityBanner } from "./tool-activity-banner";
import {
...
const PurePreviewMessage = ({
      ...
      if (part.type === "reasoning") {
        ...
        if (part.text?.trim().length > 0) {
          return {
            ...
            isStreaming: isStreaming || acc.isStreaming,
            rendered: false,
          };
        }

        if (isStreaming) {
          return { ...acc, isStreaming: true };
        }
      }
      return acc;
    },
  ...
  const parts = message.parts?.map((part, index) => {
    ...
    const modelReasoningBeforeTool = extractReasoningBeforeTool(
      message.parts,
      index
    );
    ...
    if (type === "reasoning") {
      if (
        !mergedReasoning.rendered &&
        (mergedReasoning.text || mergedReasoning.isStreaming)
      ) {
      ...
      if (
        ...
        return (
          <div className="w-[min(100%,520px)]" key={toolCallId}>
            <AssistantToolCall
              input={part.input}
              modelReasoning={modelReasoningBeforeTool}
              state={state}
    ...
    if (type === "dynamic-tool" || type.startsWith("tool-mcp_")) {
